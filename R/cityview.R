...
cityview <- function(name, zoom = 1,
  ...
  int_p <- int_p + ggplot2::geom_sf(data = obj[["lines"]], color = colors[["rails"]], size = 0.35, inherit.aes = FALSE)
  .tick(progBar, verbose)
  # Small streets ##############################################################
  ...
  int_p <- int_p + ggplot2::geom_sf(data = obj[["lines"]], color = colors[["lines"]], size = 0.55, inherit.aes = FALSE)
  .tick(progBar, verbose)
  obj <- .get_features(osmbox, cropped, border, features = "\"highway\"=\"tertiary\"")
  int_p <- int_p + ggplot2::geom_sf(data = obj[["polygons"]], fill = NA, color = colors[["lines"]], size = 0.6, inherit.aes = FALSE)
  int_p <- int_p + ggplot2::geom_sf(data = obj[["lines"]], color = colors[["lines"]], size = 0.55, inherit.aes = FALSE)
  .tick(progBar, verbose)
  ...
  int_p <- int_p + ggplot2::geom_sf(data = obj[["polygons"]], fill = NA, color = colors[["lines"]], size = 0.7, inherit.aes = FALSE)
  int_p <- int_p + ggplot2::geom_sf(data = obj[["lines"]], color = colors[["lines"]], size = 0.7, inherit.aes = FALSE)
  .tick(progBar, verbose)
