...
You can select different themes for the plot. The images above are created using `theme = "original"`, but other options for the `theme` argument include `light` (top left), `dark` (top middle), `X` (top right), `rouge` (bottom left), `colored` (bottom middle), and `neon` (bottom right).
